...
Fields are comma-separated (`key=value,key=value`), so a value that itself
contains a comma — a `system_prompt` with punctuation, for example — must be
quoted, or the comma will be read as the start of a new field:

```bash
lighteval vllm \
    'model_name=gpt2,system_prompt="Be helpful, concise, and friendly"' \
    gsm8k
```

For anything more than one or two fields, a YAML config file
(`lighteval vllm my_config.yaml ...`) avoids this class of issue entirely and
...
That controls the *output* side (stripping thinking tokens after generation).
To control the *input* side — whether the model thinks at all — pass
`chat_template_kwargs` in your model config. It forwards arbitrary keyword
arguments to the tokenizer's `apply_chat_template`, which is how models like
Qwen3 expose an `enable_thinking` switch in their chat template:
...
This is supported by every backend that renders prompts with a local
tokenizer (`accelerate`/`transformers`, `vllm`, `sglang`, and the
`endpoint`/`tgi` backends). It has no effect on backends that never apply a
chat template locally (e.g. `litellm`), since those send raw messages to the
